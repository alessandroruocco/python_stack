...
def morlet2(M, s, w=6):
    ...
    should be set to `numpy.complex128` for best results.
...
def cwt(data, wavelet, widths, dtype=None, **kwargs):
    ...
    by a width parameter and length parameter [1]. The `wavelet` function
    ...
    dtype : data-type, optional
        ...
        output of `wavelet` is real and `data` is real. If either one of
        them is complex, `dtype` is set to ``complex128``.
    ...
    ::
        ...
        cwt[ii,:] = signal.fftconvolve(data, np.conj(wavelet(length, width[ii],
                                        **kwargs))[::-1], mode='same')

    References
    ----------
    .. [1] : S. Mallat, *A Wavelet Tour of Signal Processing*, 3rd Edition:
             The sparse way, Academic Press, 2008, pg. 112.
    ...
    if dtype is None:
        wavelet_type = np.asarray(wavelet(1, widths[0], **kwargs)).dtype.char
        data_type = np.asarray(data).dtype.char
        if wavelet_type in 'FD' or data_type in 'FD':
            dtype = np.complex128
        else:
            dtype = np.float64
