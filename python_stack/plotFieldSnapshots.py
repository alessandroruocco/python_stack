...
def plotGridData(fig,ax,data,x,y,t,symmetric,cmap,xLabel,yLabel,zLabel,
    ...
    ax.tick_params(reset=True,axis='both',color=tickColor,direction='in')
...
def plotField(fig,ax,field,x,y,t,fieldName,**plotArgs):
    ...
    plotGridData(fig,ax,field,x,y,t,True,'RdBu_r',xLabel,yLabel,zLabel,grid=True,**plotArgs)
...
def plotFieldEnv(fig,ax,field,x,y,t,fieldName,**plotArgs):
    ...
    plotGridData(fig,ax,fieldEnv,x,y,t,False,'viridis',xLabel,yLabel,zLabel,
                 grid=True,**plotArgs)
...
def plotFieldFT(fig,ax,field,x,y,t,fieldName,Te=None,ne=None,windowFunc=np.hanning,**plotArgs):
    ...
    plotGridData(fig,ax,fieldFT**2,xwns,ywns,t,False,'inferno',xLabel,yLabel,zLabel,
...
def plotFieldFTy(fig,ax,field,x,y,t,fieldName,Te=None,ne=None,windowFunc=np.hanning,**plotArgs):
    ...
    plotGridData(fig,ax,fieldFTy**2,x,ywns,t,False,'inferno',xLabel,yLabel,zLabel,
...
def plotFieldWltx(fig,ax,field,x,y,t,fieldName,Te=None,ne=None,
    ...
    plotGridData(fig,ax,fieldWltx,x,xwns,t,False,'inferno',xLabel,yLabel,zLabel,
                 aspect='auto',grid=False,cbOrientation='vertical',
                 xLabelPos='bottom',tickColor='w',**plotArgs)
...
def expDensity(x,n0,Ln):
    return n0*np.exp(x/Ln)

def processDensity(density,x,y,densityName,subtract=False,frac=1.0,ne=None):
...
def plotDensity(fig,ax,data,x,y,t,densityName,subtract=False,frac=None,ne=None,**plotArgs):
    ...
    plotGridData(fig,ax,density,xCC,yCC,t,symmetric,cmap,xLabel,yLabel,zLabel,**plotArgs)
...
def animDensity(fig,ax,sdfFiles,x,y,densityName,subtract=False,frac=None,ne=None,**plotArgs):
    ...
    procDataFunc = lambda d: processDensity(d,x,y,densityName,subtract,frac,ne)
...
def processDensityFT(density,x,y,densityName,windowFunc,subtract=False,frac=1.0,ne=None):
    density,_,_ = processDensity(density,x,y,densityName,subtract=subtract,
                                 frac=frac,ne=ne)
    xCC = 0.5*(x[1:] + x[:-1])
    yCC = 0.5*(y[1:] + y[:-1])
...
def plotDensityFT(fig,ax,density,x,y,t,densityName,subtract=False,frac=None,ne=None,Te=None,windowFunc=np.hanning,**plotArgs):
    ...
    plotGridData(fig,ax,densityFT,xwns,ywns,t,False,'inferno',xLabel,yLabel,zLabel,
                 grid=False,cbOrientation='vertical',xLabelPos='bottom',
                 tickColor='w',**plotArgs)
...
def processDensityFTy(density,x,y,densityName,windowFunc,subtract=True,frac=None,ne=None):
    density,x,_ = processDensity(density,x,y,densityName,subtract=subtract,
                                 frac=frac,ne=ne)
    yCC = 0.5*(y[1:] + y[:-1])
...
def plotDensityFTy(fig,ax,density,x,y,t,densityName,subtract=True,frac=None,ne=None,Te=None,windowFunc=np.hanning,**plotArgs):
    ...
    plotGridData(fig,ax,densityFTy,x,ywns,t,False,'inferno',xLabel,yLabel,zLabel,
                 aspect='auto',grid=False,cbOrientation='vertical',
...
def plotDensityMeanDiffY(fig,ax,density,x,y,t,densityName,subtract=False,
    ...
    density,xCC,yCC = processDensity(density,x,y,densityName,subtract,frac,ne)
...
#		ax.set_ylim(0.0,maxAmp)
    ...
    if subtract:
        ax.set_ylabel(r'$\langle\frac{\delta n}{n_0}\rangle_y$',rotation=0)
    else:
        ax.set_ylabel(r'$\langle n/n_{\mathrm{cr}}\rangle_y$',rotation=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--spatialX',type=float,nargs=2)
...
#    parser.add_argument('--spatialY',type=float,nargs=2)
    ...
    if args.animate:
        ...
        #fig.suptitle(args.dataName.replace('_','\_'))
        sdfFile = args.fName
    ...
    print(xOrig,yOrig)
    # Process cropping arguments
    # WARNING: must be done before reading data. Mistakes were made and
    # cropping variables are global. TODO: Fix this?
    _nCrop = args.nCrop
    if args.cropx is not None:
        _nCropx = (srsUtils.misc.closestMatchLoc((xOrig-xOrig[0])/1e-6,args.cropx[0])[0],
                   srsUtils.misc.closestMatchLoc((xOrig[-1]-xOrig[::-1])/1e-6,args.cropx[1])[0])
    elif args.nCropx is not None:
        _nCropx = args.nCropx
    else:
        _nCropx = (args.nCrop,args.nCrop)
    if args.cropy is not None:
        _nCropy = (srsUtils.misc.closestMatchLoc((yOrig-yOrig[0])/1e-6,args.cropy[0])[0],
                   srsUtils.misc.closestMatchLoc((yOrig[-1]-yOrig[::-1])/1e-6,args.cropy[1])[0])
    elif args.nCropy is not None:
        _nCropy = args.nCropy
    else:
        _nCropy = (args.nCrop,args.nCrop)

    print("Cropping:\n - x: {:}\n - y: {:}".format(_nCropx,_nCropy))
    ...
    print(xOrig[_nCropx[0]]*1e6,xOrig[_nCropx[1]]*1e6)

    # Crop data and x and y grid
    if not args.animate:
        t,data = getData(sdfFile,args.dataName)
    print('check cropping')
    print(sum(_nCropx),xOrig.shape[0])
    #if sum(_nCropx) >= xOrig.shape[0]:
    #    print(xOrig.shape[0])
    #    raise ValueError("Number of cells to be cropped in x is greater than array size!")
    if sum(_nCropy) >= yOrig.shape[0]:
        raise ValueError("Number of cells to be cropped in y is greater than array size!")
    if args.spatialX:
        x = xOrig[index_xlims0:index_xlims1]
    #elif _nCropx[1] == 0:
     #   x = xOrig[_nCropx[0]:]
    else:
        x = xOrig[_nCropx[0]:-_nCropx[1]]
    if _nCropy[1] == 0:
        y = yOrig[_nCropy[0]:]
    else:
        y = yOrig[_nCropy[0]:-_nCropy[1]]
    print(x.max()*1e6,xOrig.max()*1e6)
    ...
    if args.densityProfile == 'exp':
        Ln = (xOrig[-1]-xOrig[0])/math.log(args.densityRange[1]/args.densityRange[0])
        xCC = 0.5*(x[1:] + x[:-1])
        ne = expDensity(xCC,args.densityRange[0],Ln)
        ...
        ne = sdf.read(initSnapFName).Derived_Number_Density_electrons.data
    ...
    if args.animate:
        if args.parallelAnim: plotArgs['parallel'] = True
        if args.cache:
            plotArgs['cache'] = True
            plotArgs['cacheDir'] = args.cacheDir
            plotArgs['useExistingCache'] = True


    # Now create figure and axis objects and set size of figure required
    ...
    if args.spatialX and args.plotType != 'FT':
    #if args.spatialX:
    ...
    if 'Field_' in args.dataName:
        if args.animate:
            if args.cache and plotArgs['cacheDir'] is None:
                plotArgs['cacheDir'] = os.path.join(args.fName,
                                           'processed/field_'+args.plotType)

            if args.plotType == 'normal':
                anim = animField(fig,ax,files,x,y,args.dataName,**plotArgs)
                #anim = animField(ax,files,args.dataName)
            elif args.plotType == 'FT':
                anim = animFieldFT(fig,ax,files,x,y,args.dataName,Te=args.temperature,ne=ne,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'FTy':
                anim = animFieldFTy(fig,ax,files,x,y,args.dataName,Te=args.temperature,ne=ne,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'wltx':
                anim = animFieldWltx(fig,ax,files,x,y,args.dataName,
                          Te=args.temperature,ne=ne,
                              wltFreqMult=args.wltx_freqMult,
                              ySkip=args.wltx_NySkip,kSkip=args.wltx_NkSkip,
                              kMax=wltx_kMax,**plotArgs)
        else:
            if args.plotType == 'normal':
                plotField(fig,ax,data,x,y,t,args.dataName,**plotArgs)
            elif args.plotType == 'env':
                plotFieldEnv(fig,ax,data,x,y,t,args.dataName,**plotArgs)
            elif args.plotType == 'FT':
                plotFieldFT(fig,ax,data,x,y,t,args.dataName,Te=args.temperature,ne=ne,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'FTy':
                print(data.shape)
                plotFieldFTy(fig,ax,data,x,y,t,args.dataName,Te=args.temperature,ne=ne,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'wltx':
                plotFieldWltx(fig,ax,data,x,y,t,args.dataName,
                          Te=args.temperature,ne=ne,
                              wltFreqMult=args.wltx_freqMult,
                              ySkip=args.wltx_NySkip,kSkip=args.wltx_NkSkip,
                              kMax=wltx_kMax,**plotArgs)
        ...
        if args.subtractDensity:
            plotArgs['frac'] = args.densityFrac
            plotArgs['ne'] = ne

        if args.animate:
            if args.cache and plotArgs['cacheDir'] is None:
                plotArgs['cacheDir'] = os.path.join(args.fName,
                                           'processed/density_'+args.plotType)

            if args.plotType == 'normal':
                anim = animDensity(fig,ax,files,x,y,args.dataName,**plotArgs)
            elif args.plotType == 'FT':
                anim = animDensityFT(fig,ax,files,x,y,args.dataName,Te=args.temperature,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'FTy':
                anim = animDensityFTy(fig,ax,files,x,y,args.dataName,Te=args.temperature,windowFunc=np.hanning,**plotArgs)
        else:
            if args.plotType == 'normal':
                plotDensity(fig,ax,data,x,y,t,args.dataName,grid=True,**plotArgs)
            elif args.plotType == 'FT':
                plotDensityFT(fig,ax,data,x,y,t,args.dataName,Te=args.temperature,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'FTy':
                plotDensityFTy(fig,ax,data,x,y,t,args.dataName,Te=args.temperature,windowFunc=np.hanning,**plotArgs)
            elif args.plotType == 'meanDiffY':
                plotDensityMeanDiffY(fig,ax,data,x,y,t,args.dataName,grid=True,**plotArgs)

    # Save output
    if args.animate:
        mywriter = animation.FFMpegFileWriter(bitrate=-1,codec='libx264',fps=10)
        #mywriter = animation.FFMpegWriter(bitrate=-1,codec='libx264',fps=10)
        anim.save(args.output,writer=mywriter,dpi=300)
    else:
